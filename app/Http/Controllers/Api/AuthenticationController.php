...
class AuthenticationController extends Controller
{
    ...
    public function deleteAccount(Request $request): EmptySuccessfulResponseResource
    {
        ...
        if ($request->user()->unfinishedOrders()->exists()) {
            throw new AuthenticationException('User has unfinished order');
        }
    ...
    public function sendOtp(Request $request): EmptySuccessfulResponseResource
    {
        ...
        try {
            ...
            Log::emergency('Authentication api: OTP Error: '.$e->getMessage());
            throw new AuthenticationException(
                trans('auth.something_went_wrong'),
    ...
    public function sendVerifyEmailToEmail(User $user, string $email, bool $isUpdate = false): void
    {
        ...
        try {
            ...
            Log::emergency('Authentication api: Email Error: '.$e->getMessage());
            throw new AuthenticationException(
                trans('auth.something_went_wrong'),
    ...
    {
        if ($keyLength <= 0) {
            Log::emergency('Authentication api: PIN code length must be positive.');
