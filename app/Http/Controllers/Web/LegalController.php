...
class LegalController extends Controller
{
    ...
    public function createSupport(Request $request)
    {
        ...
        if (RateLimiter::tooManyAttempts($key, 3)) {
            Log::channel('app_log')->warning('LegalController: Rate limit reached', ['ip' => $ip]);
        ...
        Log::channel('app_log')->info('LegalController: New support request', ['ip' => $ip, 'email' => $validated['email']]);

        if (!empty($validated['website'])) {
            RateLimiter::hit($key, 3600);
            Log::channel('app_log')->warning('LegalController: Validation website field error', ['ip' => $ip]);
            return back()->with('error', 'حدث خطأ أثناء إرسال الرسالة. حاول مرة أخرى لاحقاً.');
        }
        if (!$this->verifyEmailDomain($validated['email'])) {
            Log::channel('app_log')->warning('LegalController: Email domain not found', ['ip' => $ip, 'email' => $validated['email']]);
            return back()->with('error', 'حدث خطأ أثناء إرسال الرسالة. حاول مرة أخرى لاحقاً.');
        }

        //TODO spam detection using https://akismet.com and https://github.com/josiasmontag/laravel-recaptchav3
        Log::channel('app_log')->debug('LegalController: spam detection is not supported yet');
        ...
        try {
            ...
            Log::channel('app_log')->warning('LegalController: error creating support', ['ip' => $ip, 'error' => $e->getMessage()]);
            return back()->with('error', 'حدث خطأ أثناء إرسال الرسالة. حاول مرة أخرى لاحقاً.');
        }
    }
