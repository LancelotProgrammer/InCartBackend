...
enum CouponType: int implements HasLabel
{
    ...
    private function calculateTimedDiscount(CouponService $context, Coupon $coupon): float
    {
        ...
        Log::channel('app_log')->info("Enums: Calculating discount for coupon with id: {$couponId}", [
            'config' => $config
        ]);

        $validator = Validator::make($config, $this->getValidationRulesForApply());
        if ($validator->fails()) {
            Log::channel('app_log')->critical("Invalid config for coupon with id: {$couponId}", [
        ...
        Log::channel('app_log')->info("Enums: Coupon with id: {$couponId} is valid.");
